...
function uploadBody({
  ...
  // ФАЙЛОВ МОЖЕТ БЫТЬ НЕСКОЛЬКО (ВОИС шлёт два за месяц, 25.09.2026) —
  // сервер соберёт их в один отчёт. Один файл уходит так же, списком из одного.
  for (const f of Array.isArray(file) ? file : [file]) body.append('files', f);
