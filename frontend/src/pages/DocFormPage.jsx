...
export function DocFormPage() {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      // Ошибку показываем заметным окном во весь экран (оверлей), а не мелкой
      // подписью внизу — оператор мог её не замечать. Текст уже человеческий
      // (см. field_label_for на бэкенде: «Не заполнены обязательные поля: …»).
      openModal('alert', { title: 'Не удалось сформировать документ', message: e.message });
    } finally {
      setBusy(false);
    }
  }
