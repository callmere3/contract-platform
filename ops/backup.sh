#!/bin/bash
#
# Бэкап contract-platform на самом VPS — ТЕПЕРЬ ТОЛЬКО РУКАМИ.
#
# С 26.09.2026 ИЗ CRON УБРАН (решение владельца): с архивом отчётов Dista
# база весит гигабайты, и 14 дампов на диске в 25 ГБ не помещаются.
# Ежедневная копия теперь идёт потоком прямо на ПК (ops/serve-backup.sh +
# ops/pull-backup.ps1) и на сервере не хранится. Этот скрипт остался для
# разового снимка перед рискованной правкой (миграция, массовый UPDATE):
# запустить, сделать дело, удалить файлы из /root/backups — и следить за
# местом на диске. Описание ниже — как было задумано изначально.
#
