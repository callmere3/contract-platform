...
# Поля, из которых складывается ПОЛНОТА карточки (_contragent_is_complete).
# Правка одного из них — это и есть сбор данных; переименование карточки или
# смена типа договора — нет.
#
# Список приходится держать здесь отдельно от самой проверки: та смотрит на
# ЗНАЧЕНИЯ готовой карточки (c.name, c.country, …), а нам нужны ИМЕНА полей
# из запроса на правку. Разъехаться они не должны — если в полноту добавят
# поле, добавить его надо и сюда.
...
def _is_data_work(action: str, meta: dict) -> bool:
    """
    Была ли эта запись журнала СБОРОМ ДАННЫХ, а не просто касанием карточки.

    Создание карточки — да, всегда: её заводят, заполняя форму.

    Правка — только если меняли поле, от которого зависит полнота
    (COMPLETENESS_FIELDS). Переименование титла, смена типа договора или
    правка псевдонимов карточку полнее не делают, а достижение раньше
    засчитывало и их: у админа оно появилось за две правки титла, хотя ни
    одного поля он не заполнял (17.09.2026, разбор по просьбе владельца).

    Записи БЕЗ списка полей (сделанные до этой правки) не засчитываются: что
    в них меняли — неизвестно, а догадываться в пользу значка значит оставить
    ровно ту ошибку, ради которой всё и переделывалось. Потерять при этом
    нечего: правок карточек после сброса зачёта было всего две, обе
    админские и обе как раз титульные.
    """
    if action == "contragent.create":
        return True
    fields = meta.get("fields")
    if not isinstance(fields, list):
        return False
    return bool(COMPLETENESS_FIELDS.intersection(fields))


def filled_cards(db: Session, user_id) -> int:
    """
    Сколько карточек контрагентов, которых человек касался ПО ДЕЛУ, заполнены
    полностью ПРЯМО СЕЙЧАС.

    «По делу» — это заведение карточки или правка поля, от которого зависит
    полнота (см. _is_data_work). Переименование титла сюда не входит.
    ...
    for row in (
        ...
        meta = row.meta or {}
        if meta.get(SOURCE_KEY) == SOURCE_SCRIPT:
            continue
        if not _is_data_work(row.action, meta):
            continue
...
#
# Admin здесь побывал один день — владелец смотрел, как это выглядит, — и
# убран по его же просьбе 19.09.2026.
HONORARY_USERNAMES = ("Egor",)
HONORARY_NOTE = (
