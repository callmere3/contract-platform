...
def list_contragent_templates(
    ...
    Сравнение — по ДВУМ тегам: country и contragent_type. contract_family
    (тип договора) в подбор НЕ входит (изменено по просьбе пользователя):
    контрагент не привязан к одному семейству, поэтому по его стране и типу
    отдаём ВСЕ подходящие документы — и роялти, и аванс, и аванс с
    обязательством, — а оператор выбирает нужный. Раньше сравнение шло по
    трём тегам, и контрагент видел документы лишь одного семейства.
    ...
    где-то закрадётся несовпадающий регистр country/type, документ просто не
    найдётся без явной ошибки — держать в голове при отладке.
