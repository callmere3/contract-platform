...
Эндпоинты работы с контрагентами (этап 4, база контрагентов).
  ...
  POST /contragents                    — создать контрагента (title/contract_number
                                          ...
                                          409 при совпадении титла с существующей
                                          карточкой, обход — confirm_duplicate=true)
...
def _title_key(value: str | None) -> str:
    """
    Ключ сравнения титлов при проверке дублей: регистр и ВСЕ пробелы
    отбрасываются. Пробелы убираются целиком, а не схлопываются, потому что
    в базе реально лежат оба написания — «Куликов И.М. (СГ)» и
    «Куликов И.М.(СГ)» — и это один и тот же человек, заведённый дважды.
    """
    return "".join((value or "").split()).casefold()


def _find_title_duplicates(db: Session, title: str) -> list[Contragent]:
    """
    Карточки, чей титл совпадает с `title` с точностью до регистра и пробелов.

    Сравниваем именно ТИТЛ, а не name: 465 из 828 боевых карточек пришли
    импортом с пустым name, и проверка по ФИО против них не работает в
    принципе — ровно так 09.09.2026 создался дубль «Кеосеян Э. З. (ИП)»
    (см. create_contragent). Титл же есть у каждой карточки, а форма
    создания вычисляет его из ФИО+типа той же build_contragent_title —
    значит, вычисленный титл новой карточки сравним с титлом импортной.

    Кандидаты сужаются по первому слову титла (фамилия/начало названия),
    чтобы не тянуть в память всю базу; итоговое сравнение — точное, по
    ключу, поэтому спецсимволы ILIKE в этом слове ничего не ломают: они
    дадут лишь чуть более широкую выборку кандидатов.
    """
    key = _title_key(title)
    if not key:
        return []
    words = title.split()
    token = words[0] if words else title
    rows = db.query(Contragent).filter(Contragent.title.ilike(f"%{token}%")).all()
    return [c for c in rows if _title_key(c.title) == key]
...
def create_contragent(
    ...
    confirm_duplicate: bool = Form(False),  # «да, создать, хотя такой титл уже есть»
    db: Session = Depends(get_session),
    ...
    но если передан — проверяется ТОЛЬКО на цифры и длину под contragent_type
    (см. app/tags.py: REG_NUMBER_META). На уникальность НЕ проверяется: UNIQUE
    с него снят 11.08.2026, карточки на аванс и роялти у одного человека
    легально делят один ИНН/ОГРНИП. Точный уникальный идентификатор
    контрагента — dista_id (models.py), и через эту форму он не задаётся.
    ...
    ЗАЩИТА ОТ ДУБЛЕЙ (переделана 10.09.2026 после инцидента). Раньше она
    жила только на фронте: тот искал GET /contragents?q=<полное ФИО> и
    блокировал сабмит при точном совпадении c.name. Обе половины давали
    осечку на импортных карточках — у них name пустой, а title сокращённый
    («Кеосеян Э. З. (ИП)»), и подстроки «Кеосеян Эдгар Зареевич» в нём нет:
    поиск не возвращал НИЧЕГО, дубль создавался молча. Теперь проверка
    здесь, на сервере, и сравнивает ВЫЧИСЛЕННЫЙ ТИТЛ (_find_title_duplicates).

    Совпадение титла — не запрет, а требование подтвердить: «Иванов И. И.
    (СГ)» могут быть два РАЗНЫХ человека (титл — это фамилия + инициалы +
    тип), да и вторая карточка на того же человека под аванс/роялти законна.
    Поэтому 409 с detail={code, message, duplicates:[{id,title}]}, а
    повторный запрос с confirm_duplicate=true создаёт карточку и пишет в
    audit_log, с чем именно оператор согласился.

    Чего проверка НЕ ловит: дубль, чей титл записан в другом формате («ООО
    Золотой звук» против вычисленного «Золотой звук (ООО)») — ключ сравнения
    строгий. Это остаётся на мягкой подсказке «похожие уже есть» в форме
    (она ищет по первому слову, а не по полному ФИО).
    ...
    duplicates = _find_title_duplicates(db, title)
    if duplicates and not confirm_duplicate:
        raise HTTPException(
            ...
            detail={
                ...
                "message": (
                    f"Карточка «{title}» уже есть в базе. Если это другой человек "
                    f"или вторая карточка того же (аванс/роялти) — подтвердите создание."
                ),
                "duplicates": [{"id": str(c.id), "title": c.title} for c in duplicates],
            },
        )
    ...
    try:
        ...
        # Уникальных индексов, в которые могла бы упереться эта форма, сейчас
        # нет (reg_number потерял UNIQUE 11.08.2026, dista_id тут не задаётся),
        # так что ветка практически недостижима. Оставлена, чтобы будущий
        # constraint не превратился в 500 с трейсбеком: раньше здесь висело
        # сообщение про дубль рег. номера — неверное с 11.08.2026.
        db.rollback()
        raise HTTPException(
            status_code=400,
            detail="Не удалось сохранить контрагента: конфликт данных в базе.",
        )

    # Создание «поверх» существующего титла фиксируем отдельно: это ровно тот
    # след, которого не хватило при разборе дубля 09.09.2026 — по журналу было
    # не понять, видел ли оператор предупреждение.
    meta = {"title": contragent.title}
    if duplicates:
        meta["duplicate_confirmed"] = [c.title for c in duplicates]
    log_action(
