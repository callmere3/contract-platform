...
def _owner_pairs(s: Settings, *, positive: bool = False):
    ...
    НЕКАТАЛОГ СЧИТАЕТСЯ НАРАВНЕ С КАТАЛОГОМ (решение владельца 26.09.2026,
    отмена правила от 25.09). В Dista изъятые позиции входили в начисления:
    за II кв. 2026 её акты сходятся до копейки только вместе с неКаталогом
    (1,89 млн ₽ вознаграждения, 99 правообладателей). Отбор «только
    каталог» лишил бы их этих денег.
    """
    parts = []
