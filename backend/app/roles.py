...
# ДЕНЬГИ ПО КОНТРАГЕНТАМ — отдельно от доступа к продукту (19.09.2026,
# просьба владельца «убрать балансы у финансового менеджера»). Список
# контрагентов и их карточки он видит: это справочник, с которым он работает
# в номенклатуре; а сколько кому выплачено — нет.
#
# Почему отдельным правом, а не «если не финансовый менеджер»: balance у
# карточки — самостоятельное знание, и закрывать его однажды придётся не
# только этой роли.
CAN_VIEW_BALANCES = (ADMIN, DIRECTOR)
...
# Выгрузить каталог в Excel — оба, как и экспорт контрагентов, плюс
# финансовый менеджер: смотреть каталог он и так может, а выгрузка — то же
# чтение, только файлом (просьба владельца 19.09.2026: «оставь только
# экспорт»). ЗАЛИВАТЬ каталог ему по-прежнему нельзя — см. ниже.
CAN_EXPORT_NOMENCLATURE = (ADMIN, DIRECTOR, FINANCE_MANAGER)
...
# А ЗАЛИВАТЬ ФАЙЛОМ — нет (просьба владельца 19.09.2026). Импорт правит
# справочник целиком и молча: переименовывает по коду, проставляет коды по
# имени — сотню строк разом, и увидеть это можно только постфактум. Завести
# и переименовать руками он по-прежнему может, выгрузить — тоже.
CAN_IMPORT_PARTNERS = (ADMIN, DIRECTOR)
...
# Генерация отчётов правообладателям (24.09.2026): из отчётов площадок и прав
# на треки считается, сколько причитается каждому правообладателю, и
# собираются ведомости. Право СВОЁ — это уже не «что пришло», а «сколько
# отдать», и однажды его отдадут бухгалтеру, которому незачем грузить отчёты.
CAN_GENERATE_ROYALTY_REPORTS = (ADMIN, DIRECTOR)
...
# ДОСТИЖЕНИЯ И КУБОК — про работу с документами, поэтому у ролей ML Docs.
# Финансовому менеджеру их не за что получать: он документов не делает, и
# пустой раздел с серыми плитками в его профиле объяснял бы ему, чего он
# лишён, вместо того чтобы помогать.
CAN_HAVE_ACHIEVEMENTS = DOCS_ROLES
