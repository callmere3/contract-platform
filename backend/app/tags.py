...
def build_article(country: str | None, reg_number: str | None) -> str | None:
    """
    Артикул контрагента = страна + рег.номер (напр. 'РУ111111111111',
    'KZ860830300123'). Задуман как ЕДИНСТВЕННЫЙ уникальный идентификатор
    контрагента в будущем; пока показывается в карточке (только админу).

    Вычисляется на лету из country и reg_number, отдельной колонки нет:
    reg_number уже unique, а страна лишь префикс — хранить производное
    значение и держать его в синхроне не нужно. Если страны или рег.номера
    нет — артикул не сформировать (карточку ещё дозаполняют), возвращаем None.
    """
    if not country or not reg_number:
        return None
    return f"{country}{reg_number}"
...
# Метки, которые в шаблонах обычно берут из карточки контрагента. Нужны ТОЛЬКО
# для порядка полей в модалке настройки шаблона: эти поля поднимаются наверх
# списка, потому что настраивают всегда их, а искать их в середине третьего
# десятка меток неудобно (просьба владельца 17.09.2026). Ни на проверку, ни на
# подстановку это не влияет — источник по-прежнему задаёт админ вручную.
#
# Значение — источник, который сюда обычно и ставят: на будущее, если
# понадобится предлагать его одним нажатием.
CARD_FIELD_HINTS = {
    "name": "contragent.name",
    "inn": "contragent.reg_number",
    "ogrnip": "contragent.reg_number",
    "ogrn": "contragent.reg_number",
    "bin": "contragent.reg_number",
    "royalty": "contragent.royalty_percent",
    "contract": "contragent.contract_number",
    "nickname": "contragent.nickname",
}

# ШАБЛОНЫ, КОТОРЫХ ЕЩЁ НЕ ВИДЕЛ ЮРИСТ (17.09.2026). Над формой генерации
# висит красное предупреждение, и человек знает, что документ надо вычитать.
# Список — СЕМЕЙСТВАМИ договоров, а не id шаблонов: сейчас это ровно четыре
# договора «с обязательством» (СГ и ИП × аванс и роялти), и id у них в базе
# разные на каждом стенде.
#
# Приложения и акты сюда не входят: у них своя ось (бакет обязательства), они
# старые и юристом смотрены. Одобрят договоры — эта строка и удаляется.
UNAPPROVED_CONTRACT_FAMILIES = {"АВАНС_ОБЯЗАТЕЛЬСТВО", "РОЯЛТИ_ОБЯЗАТЕЛЬСТВО"}
UNAPPROVED_NOTICE = (
    "Шаблон ещё не одобрен юристом. Сгенерированный документ обязательно "
    "отправьте юристу на проверку — до того, как передавать его контрагенту."
)
