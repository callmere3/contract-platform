...
def build_document_filename(
    ...
    АКТЫ БЫВАЮТ ДВУХ ВИДОВ, и какой именно — видно по метке в шаблоне (то
    есть по данным, а не по захардкоженному списку имён шаблонов):
      appendix_no — акт К ПРИЛОЖЕНИЮ (Акт_СГ_аванс, Акт_СГ_роялти).
                    Своего номера у него нет, он привязан к приложению,
                    поэтому в имени "Акт к Пр. 1".
      act_no      — самостоятельный акт (Акт_СГ_обяз. на треки) со своим
                    номером: "Акт 1".
    Раньше здесь смотрели только act_no — и у актов к приложению номер в
    имени просто не появлялся, файл назывался "Акт".
