...
# Служебные слова названия компании, которые не считаются "значимыми" и не
# дают буквы в инициалы: союзы и предлоги. Набор намеренно маленький — только
# то, что реально встречается в названиях («Рога и копыта» -> «РК»).
_COMPANY_STOPWORDS = {
    "и", "а", "но", "в", "во", "на", "с", "со", "по", "за", "из",
    "от", "до", "для", "о", "об", "у", "к",
}


def build_company_initials(name: str, contragent_type: str) -> str:
    """
    Инициалы значимых слов названия компании для номера договора.

        build_company_initials('ТОО «Астана Медиа»', 'ТОО') -> 'АМ'
        build_company_initials('ООО «Рога и копыта»', 'ООО') -> 'РК'

    В отличие от build_initials() (ФИО физлица), здесь:
      - отбрасывается ведущая орг.форма (ООО/ТОО), если она есть в названии —
        иначе «ТОО «Астана Медиа»» дало бы «Т…» от самого «ТОО»;
      - с каждого слова снимаются кавычки и пунктуация (берём только буквы);
      - пропускаются служебные слова (союзы/предлоги, см. _COMPANY_STOPWORDS),
        чтобы «Рога и копыта» дали «РК», а не «РИК».

    Если после чистки не осталось ни одной буквы (нетипичное название) —
    вызывающая сторона откатывается на build_initials(), лучше хоть что-то.
    """
    tokens = name.split()
    # ведущая орг.форма: сравниваем без кавычек и регистра
    if tokens and tokens[0].strip("«»\"'").upper() == contragent_type.upper():
        tokens = tokens[1:]
    letters = []
    for tok in tokens:
        cleaned = "".join(ch for ch in tok if ch.isalpha())
        if not cleaned or cleaned.lower() in _COMPANY_STOPWORDS:
            continue
...
def build_contract_number(
    ...
    Собирает номер договора формата 'МЛ-01/01/26-ИИИ/СГ'.
        ...
        full_name — ФИО физлица или название компании (инициалы считаются
                    автоматически, по-разному для физлиц и компаний)
        doc_kind  — тип контрагента: 'СГ'/'ФЛ'/'ИП' (физлицо) или 'ООО'/'ТОО'
                    (компания)

    Для физлиц инициалы — первые буквы ФИО ('Иванов Иван Иванович' -> 'ИИИ').
    Для компаний (ООО/ТОО) — первые буквы значимых слов названия без орг.формы
    ('ТОО «Астана Медиа»' -> 'АМ'), см. build_company_initials.
    ...
    """
    if doc_kind in ("ООО", "ТОО"):
        # для компании — инициалы названия; на нетипичном названии, где
        # значимых слов не осталось, откат на общий build_initials
        initials = build_company_initials(full_name, doc_kind) or build_initials(full_name)
    else:
...
def build_contragent_title(name: str, contragent_type: str) -> str:
    ...
    строки подписи в документе).
        ...
        build_contragent_title('ТОО «Астана Медиа»', 'ТОО')
            -> 'ТОО «Астана Медиа» (ТОО)'

    Для компаний (ООО, ТОО) ожидается, что name уже содержит полное
    название организации (см. models.py, докстринг Contragent.name) — title
    его НЕ переформатирует в инициалы, только добавляет тег типа в скобках.
    Для физлиц (ФЛ/СГ/ИП) name — это ФИО, из которого делаются инициалы.
    ...
    """
    # Компании — название целиком; сокращать «ТОО «Астана Медиа»» до
    # инициалов (как ФИО) нельзя, вышло бы «ТОО «. М.».
    if contragent_type in ("ООО", "ТОО"):
...
COUNTRY_FILE_MARKER = {"РУ": "ML", "KZ": "KZ.ML"}
...
def build_document_filename(
    ...
    Имя готового файла (без расширения) по формуле владельца (04.08.2026):
        ...
        26_ML_ЛД_Иванов И. И. (СГ)_Пр.№1
        26_ML_ЛД_Иванов И. И. (СГ)_Акт№2 к Пр.№1     (акт с обязательством)
        26_ML_ЛД_Иванов И. И. (СГ)_Акт к Пр.№1       (акт без обязательства)
    ...
    для KZ (COUNTRY_FILE_MARKER); страна из карточки, а без карточки — из
    ...
    Старая формула (до 04.08.2026):
        ...
        Иванов И. И. (СГ) - Договор
        Иванов И. И. (СГ) - Приложение 1
        Иванов И. И. (СГ) - Акт к Пр. 1        (акт к приложению)
        Иванов И. И. (СГ) - Акт 1              (самостоятельный акт)

    Номер — только у Приложения и Акта: у Договора такого поля нет, его
    номер и есть номер договора.

    АКТЫ БЫВАЮТ ДВУХ ВИДОВ, и какой именно — видно по меткам в шаблоне (то
    есть по данным, а не по захардкоженному списку имён шаблонов). Все акты
    привязаны к приложению (appendix_no есть всегда), поэтому в имени всегда
    "к Пр.№N"; отличается лишь наличие СВОЕГО номера акта:
      только appendix_no       — акт БЕЗ обязательства (Акт_СГ/ИП/ООО): своего
                                 номера нет -> "Акт к Пр.№1".
      appendix_no + act_no     — акт С обязательством (Акт_СГ_обяз. на треки):
                                 есть свой номер -> "Акт№2 к Пр.№1".

    Номера договора в имени НЕТ (убран 17.07.2026): с ним выходило
    "Иванов И. И. (СГ) - Приложение 1 МЛ-01.01.26-ИИИ.СГ.docx" — длинно, а
    номер и так стоит в шапке самого документа. Из-за этого у двух Договоров
    одного человека (разные contract_family — аванс и роялти) имена файлов
    совпадут; различать их можно титлом, он для того и правится вручную
    (см. update_contragent: "Иванов И. И. (СГ, аванс)").
    ...
    # тип и номер — только у Приложения/Акта (формат по просьбе владельца
    # 11.08.2026): приложение -> «Пр.№N»; акт -> «Акт№K к Пр.№N», а если у акта
    # своего номера нет (акт без обязательства несёт только appendix_no) —
    # «Акт к Пр.№N».
    suffix = ""
    if doc_type == "appendix":
        number = str(data.get("appendix_no") or "").strip()
        suffix = f"Пр.№{number}" if number else "Пр."
    elif doc_type == "act":
        appendix_no = str(data.get("appendix_no") or "").strip()
        act_no = str(data.get("act_no") or "").strip()
        # act_no есть только у актов с обязательством (…_обяз); у актов без
        # обязательства только appendix_no — тогда номер акта не показываем.
        act_part = f"Акт№{act_no}" if act_no else "Акт"
        suffix = f"{act_part} к Пр.№{appendix_no}" if appendix_no else act_part
...
# Даты окончания квартала (ДД.ММ) — по требованию юриста Срок указывается
# до конца соответствующего квартала. Числовой формат ДД.ММ.ГГГГ (принятый в
# России), год приписывается в build_term_end.
QUARTER_ENDS = {
    1: "31.03",
    2: "30.06",
    3: "30.09",
...
def advance_parts_problem(raw: dict) -> str | None:
    """
    СВЕРКА ЧАСТЕЙ АВАНСА С ОБЩЕЙ СУММОЙ — при формировании документа
    (просьба владельца 17.09.2026). None — всё сходится.

    Части выплаты это разбивка одной и той же суммы, и договор, где они с
    ней не сходятся, противоречит сам себе: в п. 2.2 стоит «задаток в
    размере 150 000», а ниже перечислено 135 000. Поймать это глазами
    трудно — числа стоят в разных абзацах.

    Проверка живёт ЗДЕСЬ, а не в форме: форма делит сумму пополам сама, и
    разойтись числа могут только после ручной правки — то есть в любой
    момент до нажатия «Сформировать». Один и тот же расчёт в двух местах
    рано или поздно разъедется, а пропустить его нельзя ни при генерации из
    интерфейса, ни при прямом вызове API.

    Молчим, когда:
      - общая сумма не заполнена — про неё скажет проверка обязательных
        полей, и второе сообщение про «части не сходятся с пустым» только
        сбивало бы с толку;
      - сумма или часть не число — про это точнее скажет _parse_amount
        («Сумма должна быть целым числом»), когда контекст будет собираться.
    """
    total_raw = str(raw.get("advance") or "").strip()
    if not total_raw:
        return None
    try:
        ...
        parts = []
        for part in ADVANCE_PARTS:
            value = str(raw.get(f"advance_{part}") or "").strip()
            parts.append(_parse_amount(value) if value else 0)
    except ValueError:
        return None
    ...
    if paid == 0:
        return (
            "Аванс не разбит на части: укажите, сколько выплачивается до сдачи "
            f"треков. Общая сумма аванса — {build_amount_spaced(total)} ₽."
        )
    shown = " + ".join(build_amount_spaced(p) for p in parts if p)
    return (
        f"Части аванса не сходятся с общей суммой: {shown} = "
        f"{build_amount_spaced(paid)} ₽, а сумма аванса — "
        f"{build_amount_spaced(total)} ₽."
    )


# Во сколько раз штраф за непереданный трек больше его «стоимости» (доли
# аванса, приходящейся на один трек). 1.5 — решение владельца 17.09.2026:
# штраф, равный стоимости, нарушителю ничего не стоит.
PENALTY_RATE = 1.5
...
def resolve_penalty_raw(raw: dict) -> str:
    ...
    считаем по умолчанию как ПОЛТОРА «стоимости» трека: аванс / количество
    треков × 1.5, с округлением до целого рубля.

    ПОЛТОРА, А НЕ ОДНА (правка 17.09.2026, владелец: «во всех авансовых
    договорах штраф не 100%, а 150% от реальной стоимости трека»). Штраф,
    равный стоимости, ничего не стоит нарушителю: не передал трек — вернул
    ровно то, что за него получил. Половина сверху и делает его штрафом.
    Оператор в любой момент может поправить сумму вручную, если по факту
    согласована другая.
    ...
    if count_n == 0:
        return ""
    return str(round(advance_n / count_n * PENALTY_RATE))
...
def build_term_end(document_date_raw) -> str:
    ...
    Формат — ДД.ММ.ГГГГ (принятый в России), напр. '31.12.2025'.

    Пример: документ от 01.12.2020 -> +5 лет = 01.12.2025, это Q4
    -> '31.12.2025' (год — год даты+5, а не год исходного документа: если
    оригинал в январе, а +5 лет улетает в декабрь того же +5 года, это всё
    ещё тот год, никакого дополнительного сдвига).
...
def build_context(
    ...
    context.update({
        ...
        # ставка НДС прописью (шаблоны ИП/ООО, Финансовое соглашение). vat —
        # choice-поле формы ('0'/'5'/'7'/'22'), vat_text — его пропись с
        # правильным падежом слова «процент»: '22' -> «двадцать два процента».
        # Та же функция, что и для роялти (число + слово «процент»), поэтому
        # переиспользуем её. При vat='0' значение шаблоном не печатается
        # (ветка «освобожден от уплаты НДС»), но считаем без исключений.
        "vat_text": build_royalty_text(raw.get("vat")),
