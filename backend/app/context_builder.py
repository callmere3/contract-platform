...
# Сотни в родительном — понадобились количеству треков: обязательство на
# полторы сотни позиций встречается, на полторы тысячи — нет.
_HUNDREDS_GEN_RU = {
...
def num_to_words_ru_genitive(n: int) -> str:
    """
    Целое число 0..999 -> числительное в родительном падеже:
    10 -> 'десяти', 15 -> 'пятнадцати', 21 -> 'двадцати одного',
    150 -> 'ста пятидесяти'.

    Для оборотов «в течение N (...) дней», «не позднее N (...) дней»,
    «не менее N (...) Треков» — все они требуют родительного падежа.
    """
    if not (0 <= n <= 999):
        raise ValueError("num_to_words_ru_genitive принимает только 0..999")
    hundreds, rest = divmod(n, 100)
    words = []
    if hundreds:
        words.append(_HUNDREDS_GEN_RU[hundreds])
    if rest or not hundreds:
        words.append(_below_hundred_genitive(rest))
    return " ".join(words)


def _below_hundred_genitive(n: int) -> str:
    """0..99 в родительном падеже — общая часть для сотен и для чисел меньше ста."""
    if n == 0:
...
def build_count_text(count_raw) -> str:
    """
    Количество будущих треков/объектов прописью, В РОДИТЕЛЬНОМ ПАДЕЖЕ:
    '5' -> 'пяти' (правка 17.09.2026 по замечанию владельца).

    Падеж не украшение: во ВСЕХ шаблонах это метка стоит в обороте «не менее
    {{ count }} ({{ count_text }}) Треков» (проверено по всем пяти шаблонам,
    где она встречается), а «не менее» требует родительного — «не менее пяти
    Треков», а не «не менее пять Треков». То же самое у count_middle_text:
    считает их одна функция, и разойтись в падеже они не должны.

    Единица измерения («Треков», «Объектов») уже стоит в тексте шаблона
    рядом с меткой, здесь не добавляется. '' если не задано.

    Свыше 999 откатываемся на именительный: столько позиций в обязательстве
    не встречается, а падеж без предела — это ещё три сотни слов ради случая,
    которого нет.
    """
