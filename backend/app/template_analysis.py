...
# delivery_date (срок предоставления исходников, шаблон СГ_аванс с
# обязательством на будущие треки) — отдельная самостоятельная дата, не
# привязанная к doc_type, поэтому в базовом наборе, а не в date_fields_for.
# birthday и pas_date остаются обычными текстовыми полями — в документе
# они выводятся как есть, в формате 01.01.1990.
DATE_FIELDS = {"c_date", "delivery_date"}
