...
class PartnerPayment(Base):
    ...
    # части площадок (CHOIS, TikTok, Believe, Spotify, Аманат) платёж приходит
    # в валюте, а на счёт падает уже в рублях; сверять строку с письмом
    # площадки удобнее по валютной сумме. В расчётах НЕ участвует.
    #
    # Валюта отдельным полем: в файле поступлений она написана словом рядом с
    # суммой («доллар»), и без неё число ничего не говорит.
    currency_amount: Mapped[Decimal | None] = mapped_column(Numeric(16, 2))
