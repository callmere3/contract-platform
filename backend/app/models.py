...
class Contragent(Base):
    ...
    Осознанно НЕТ уникального constraint на title/name: защита от дублей —
    мягкая (поиск + подсказка на лету в UI), не блокирующая на уровне БД —
    без ИНН/ОГРН в минимальной модели точного идентификатора всё равно нет.
