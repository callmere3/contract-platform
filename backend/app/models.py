...
class PartnerPayment(Base):
    ...
    # НДС в платеже — СУММОЙ, а не ставкой: в поступлениях сверяют деньги, и
    # «сколько из них налог» отвечает на вопрос прямо, тогда как из ставки
    # это каждый раз пришлось бы вычислять. Ставка и так известна (22%), а
    # приходит нередко сумма, уже посчитанная площадкой.
    vat_amount: Mapped[Decimal | None] = mapped_column(Numeric(16, 2))
